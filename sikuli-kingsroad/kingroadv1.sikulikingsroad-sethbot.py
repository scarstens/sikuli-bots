...
def chooseMap():
    ...
    if exists( collection_maps[map_to_run] ):
        ...
        #popup('Whoa, found that map you were looking for ;)');
...
def chooseMode():
    ...
    if exists( collection_modes[map_mode_to_run] ):
        ...
        sleep(0.5)
        #popup('Hey man, I totally clicked on that map for you. Your welcome.')
        if exists( img_playalone ):
            click( img_playalone )
            #popup('Yup, we confirmed playing solo.')
        sleep(2)
    else:
        #poperror('Something went wrong during choosing the mode.')
        continue_run = False
        exit
...
while( running ):
    if option == op_play_type[1]:
        ...
        if False == isGameOpen():
            ...
            if False == logged_in:
                sleep(0.1)
                #popup('Not logged in yet :(')
            if maybeLogin():
                sleep(0.1) #do anything on login?
            if logged_in:
                #popup('Found you logged in!')
                closeAllPopups()
                if( moveToMapNPC() ):
                    #popup("Phew, found the map dude.")
                    if( continue_run ):
                        maybeZoomOut()
                    if( continue_run ):
                        chooseMap()
