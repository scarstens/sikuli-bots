...
collection_maps = ("adamars_sanctum.png", "inner_circle.png", "battlements.png", "map_blindmanspass-1.png")
collection_npcs = (Pattern("npc_world.png").targetOffset(-3, 23), Pattern("npc_helm.png").targetOffset(-2, 29),
                   Pattern("npc_dungeon-1.png").targetOffset(1, -70), Pattern("npc_swords.png").targetOffset(0, 27),)
collection_modes = ("mode_normal.png", "mode_heroic.png", "mode_champion.png")
collection_lockboxes = ("lockbox_bossdrop.png", "lockbox_dragon_blue.png", "lockbox_prism.png", "lockbox_skillrune.png",
                        "lockbox_heroessence.png", Pattern("locbox_xp.png").similar(0.80), "lockbox_tournament.png",
                        "lockbox_tournament_2.png")
...
def isFlashWorking():
    return exists(img_game_icon, 1)
    sleep(1)


def maybeTown():
    # setFindFailedResponse(SKIP)
    if exists(img_town, 1):
        click(img_town)
        click(img_yesconfirm)
        notify("Wrong place... back to town.")
        sleep(2)
...
def moveToMapNPC():
    ...
    if exists(img_town_statue_arm, 3):
        hover(Pattern(img_town_statue_arm).targetOffset(-110, 0))
        mouseDown(Button.LEFT)
        sleep(1)
        mouseUp(Button.LEFT)
        return True
    elif exists(img_town_window, 1):
...
while (running):
    if option == op_play_type[1]:
        ...
        if False == isGameOpen():
            openApp(
                r"C:\Program Files\Google\Chrome\Application\chrome.exe https://apps.facebook.com/kingsroadgame/ -start-maximized")
            logged_in = False
            sleep(8)
        else:
